...
function GetOmObj() {
    var s_account = "hmhproduction";

    if (document.domain.indexOf("local") > -1)
        s_account = "hmhdev";
    else if (document.domain.indexOf("dev") > -1)
        s_account = "hmhdev";
    else if (document.domain.indexOf("stg") > -1 || document.domain.indexOf("secure") > -1 || document.domain.indexOf("stage") > -1 || document.domain.indexOf(".pre.") > -1)
        s_account = "hmhstaging";
    else if (document.domain.indexOf("hmhco.com") > -1 || document.domain.indexOf("hmhco-v1.prd.techspa.com") > -1)
        s_account = "hmhproduction,hmhgmasubmultisuite";
   var s = s_gi(s_account);
   var w=window;
   //Page level meta data // 
	if (window.sc_products) {sc_productsvalue = window.sc_products;} else {sc_productsvalue = "";}
  if (window.sc_list) {sc_listvalue = window.sc_list;}else {sc_listvalue = "";}
 /************************** CONFIG SECTION **************************/
    ...
    s.linkInternalFilters = "javascript:,www.hmhco.com"
    ...
    s.visitorNamespace = "hmhbooks"
    s.trackingServer = "metrics.hmhco.com"
    s.trackingServerSecure = "smetrics.hmhco.com"
   //s.trackingServer = "hmhbooks.d1.sc.omtrdc.net"
    return clone(s);
}
 function ClearVars() {
	for (var i = 0; i < 75; i++) {
...
//function sc_sendAnalyticsEvent(a,b){
  //var s = GetOmObj();
	//if(sc_sendAnalyticsEvent === true) {
	//if(a == "click"){
	//	s.tl(this,'o',b);
	//	s.clearVars();
	//}
 // }
//};
//Help Link Tracking code for project shop//
function sc_sendAnalyticsEvent(a,b)
{
	var s = GetOmObj();
 if(a = "click")
{ 
  s.linkTrackVars = "eVar32,prop50,events";
...
//GMA LandingPage Footer Link Tracking//
function sc_GMAsendAnalyticsEvent(a,b)
{
  var s = GetOmObj();
if(a = "click"){
s.linkTrackVars = 'prop59,eVar42,events';
...
function sc_doVendorSpecficVariableMapping(){
if (sc_productsvalue) s.products = sc_productsvalue;// Products Value //
if (sc_listvalue) s.list1 = sc_listvalue; //list value
}
function clone(obj) {
...
s.getValOnce = new Function("v", "c", "e", "" + "var s=this,k=s.c_r(c),a=new Date;e=e?e:0;if(v){a.setTime(a.getTime(" + ")+e*86400000);s.c_w(c,v,e?a:0);}return v==k?'':decodeURIComponent(v)");
...
+ "=p.length?i:i+1)}return decodeURIComponent(v)");
...
/* Added as per Spira Incident IN17894 */
/*
...
/*
 */
//Check the query params for scid
var scid = s.getQueryParam('scid');

//If we have it, set it once every day
if (scid != '') {
    s.campaign = s.getValOnce(scid, 's_campaign', 1);
}
//HMHCO GMA fall promotion campaign
var gma = s.getQueryParam('gma');
if (gma != '') {
   s.campaign = s.getValOnce(gma, 's_campaign', 1);
   s.eVar47 = s.getValOnce(gma, 's_ev47', 1);
}
var elq_email = s.getQueryParam('elq_email');
var elq_cid = s.getQueryParam('elq_cid');
var elq_mid = s.getQueryParam('elq_mid');
var elq_lid = s.getQueryParam('lid');
var elq_campid = s.getQueryParam('elq_campid');
var elq_campname = s.getQueryParam('elq_campname');
var elq_emailname = s.getQueryParam('elq_emailname');

//If we have it, set it once every day
if (elq_email != '') {
    s.eVar6 = s.getValOnce(elq_email, 's_ev6', 1);
}
//If we have it, set it once every day
if (elq_cid != '') {
    s.eVar12 = s.getValOnce(elq_cid, 's_ev12', 1);
}
//If we have it, set it once every day
if (elq_mid != '') {
    s.eVar13 = s.getValOnce(elq_mid, 's_ev13', 1);
}
//If we have it, set it once every day
if (elq_lid != '') {
    s.eVar14 = s.getValOnce(elq_lid, 's_ev14', 1);
}
//If we have it, set it once every day
if (elq_campid != '') {
    s.eVar15 = s.getValOnce(elq_campid, 's_ev15', 1);
    s.campaign = s.getValOnce(elq_campid, 's_campaign', 1);
}

//If we have it, set it once every day
if (elq_campname != '') {
    s.eVar16 = s.getValOnce(elq_campname, 's_ev16', 1);
}
//If we have it, set it once every day
if (elq_emailname != '') {
    s.eVar17 = s.getValOnce(elq_emailname, 's_ev17', 1);
}

if (!s.eVar28) s.eVar28 = s.getQueryParam("DM");
s.eVar28 = s.getValOnce(s.eVar28, "s_ev28", 0);

//GMA Reference Tacking Code
if (!s.eVar57) s.eVar57 = s.getQueryParam("ref");
s.eVar57 = s.getValOnce(s.eVar57, "s_ev57", 0);

/* Added as per Spira Incident IN17894 */
s.eVar8 = s.getNewRepeat(30, 's_getNewRepeat');
s.prop13 = s.getNewRepeat(30, 's_getNewRepeat');
s.eVar63=s.pageURL=(""+s.wd.location).toLowerCase();
if(s.eVar63)s.eVar63=s.eVar63.substring(0,256); //page URL
s.prop49=s.pageURL=(""+s.wd.location).toLowerCase();
if(s.prop49)s.prop49=s.prop49.substring(0,100); //page URL
s.pageType = dl.pageType;
sc_doVendorSpecficVariableMapping();
